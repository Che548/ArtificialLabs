...
import { writeFileSync, chmodSync, copyFileSync, existsSync, mkdirSync, mkdtempSync, openSync, rmSync } from 'node:fs';
...
const app = '/Users/taras/.codex/worktrees/eb0c/ArtificialLabs/output/builds/document-ocr/app-ios/Build/Products/Debug-iphonesimulator/sfera.app';
...
const report = join(root, 'output/e2e/capacity/ios');
...
let metro, proxy;
const keyboardPreferences = join(homedir(), 'Library/Developer/CoreSimulator/Devices', device, 'data/Library/Preferences/.GlobalPreferences.plist');
const keyboardBackup = join(cert, 'qa-keyboard-preferences.plist');
...
try {
  ...
  if (!inventory.find(item => item.udid === device && item.state === 'Booted') && existsSync(keyboardPreferences)) {
    copyFileSync(keyboardPreferences, keyboardBackup);
    command('/usr/bin/plutil', ['-replace', 'AppleKeyboards', '-json', '["en_US@sw=QWERTY;hw=Automatic"]', keyboardPreferences]);
  }
  if (!inventory.find(item => item.udid === device && item.state === 'Booted')) command('xcrun', ['simctl', 'boot', device]);
  command('xcrun', ['simctl', 'bootstatus', device, '-b'], 360000);
  ...
  proxy = spawn(process.execPath, ['--import', 'tsx', 'scripts/capacity-proxy.mjs'], { cwd: root, env, stdio: ['ignore', privateLog, privateLog] });
  ...
  if (env.E2E_OCR_MATRIX_ONLY === '1') {
    ...
    for (const phase of ['ios-ocr-auth', ...nativeQaScenarios(env.E2E_QA_SCENARIOS), 'capacity-sync']) {
  ...
  if (existsSync(keyboardBackup)) copyFileSync(keyboardBackup, keyboardPreferences);
