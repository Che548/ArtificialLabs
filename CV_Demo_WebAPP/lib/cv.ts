...
export async function analyzeImage(bytes:Buffer,options:{corners?:unknown;flip?:boolean;batch:string;qr:string},abort?:AbortSignal):Promise<AnalysisResponse> {
  ...
  try {
    ...
    const analysis=await new Promise<AnalysisResponse['analysis']>((resolve,reject)=>{
      const child=spawn(cliPath(),[],{stdio:['pipe','pipe','pipe'],shell:false});let output='';let count=0;let done=false;
      const finish=(error?:Error)=>{if(done)return;done=true;clearTimeout(timer);abort?.removeEventListener('abort',cancel);if(error){child.kill('SIGKILL');reject(error);}else{try{const value=JSON.parse(output);if(value.schema_version!=='1.0'||!value.peaks||!['valid','invalid','review'].includes(value.status))throw Error();resolve(value);}catch{reject(new CvError('Анализатор вернул некорректный ответ.',502));}}};
