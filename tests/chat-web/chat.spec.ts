...
test('registration discloses AI/cloud in the existing unchecked choice', async ({
  page,
}, info) => {
  ...
  await expect(consent).toHaveAccessibleName(
    /облачную синхронизацию и ИИ Яндекс AI Studio/,
  );
  ...
  await expect(
    page
      .getByTestId('e2e-auth-consent-agreement')
      .getByText('✓', { exact: true }),
  ).toHaveCSS('opacity', '1');
  await page.screenshot({
    path: `output/playwright/registration-${info.project.name}-390.png`,
  });
...
for (const viewport of [
  ...
  test(`read-only chat renders without loops at ${viewport.width}px`, async ({
    ...
    await page
      .getByRole('button', { name: 'Добавить вложение', exact: true })
      .click();
    await expect(page.getByTestId('screen-feedback')).toBeVisible();
    await expect(
      page.getByText('Документы в профиле', { exact: true }),
    ).toBeVisible();
    await page.getByRole('button', { name: 'Понятно', exact: true }).click();
    await expect(page.getByTestId('screen-feedback')).toHaveCount(0);
    await expect(
      page.getByRole('button', { name: 'Создать синтетический QA-диалог' }),
    ).toHaveCount(0);
    expect(dialogs).toEqual([]);
    await page.getByRole('tab', { name: /^Ассистент/ }).click();
    await expect(
      page.getByText('Привет! Я Сферка', { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole('textbox', { name: 'Сообщение для Сферки' }),
    ).toHaveCount(0);
    ...
    await expect(
      page.getByRole('textbox', { name: 'Проверяемый текст документа' }),
    ).toHaveCount(0);
    await expect(
      page.getByRole('button', { name: 'Распознать на устройстве' }),
    ).toHaveCount(0);
    expect(errors).toEqual([]);
  });
}
