...
type Step = { name: string; durationMs: number; status: 'passed' | 'failed'; error?: string };
const steps: Step[] = [];
...
async function authenticatedClient(email: string, flow: 'signUp' | 'signIn') {
  ...
  assert(result.tokens?.token, `Password ${flow} did not return an access token`);
...
function fullBatch(now: number) {
  return {
    ...emptyBatch(),
    programs: [{ localId: `${runId}-program`, type: 'planning' as const, title: 'E2E программа', status: 'active' as const, startedAt: now, updatedAt: now }],
    journalEntries: [{ localId: `${runId}-journal`, occurredAt: now, kind: 'note' as const, label: 'E2E журнал', textValue: 'cross-client', source: 'manual' as const, updatedAt: now }],
    labResults: [{ localId: `${runId}-lab`, catalogKey: 'e2e-blood', title: 'E2E анализ', collectedAt: now, status: 'unreviewed' as const, analytes: [{ name: 'hCG', value: '10', unit: 'mIU/mL' }], hasLocalSourceDocument: true, updatedAt: now }],
    scanResults: [{ localId: `${runId}-scan`, testSystemKey: 'e2e-strip', capturedAt: now, confirmedValue: 'negative' as const, resultSource: 'stripcv' as const, confidence: 0.91, qualityFlags: [], algorithmVersion: 'stripcv-0.3.1', analysisStatus: 'valid' as const, signalRatio: 0.42, confirmedByUser: true, hasLocalImage: true, updatedAt: now }],
    reminders: [{ localId: `${runId}-reminder`, type: 'journal' as const, title: 'E2E напоминание', body: 'Проверить журнал', dueAt: now, updatedAt: now }],
    medicalConditions: [{ localId: `${runId}-condition`, title: 'E2E состояние', status: 'active' as const, diagnosedAt: now, notes: 'test', updatedAt: now }],
    medications: [{ localId: `${runId}-medication`, name: 'E2E препарат', dosage: '1', frequency: 'daily', active: true, updatedAt: now }],
    allergyRisks: [{ localId: `${runId}-allergy`, allergen: 'E2E аллерген', severity: 'mild' as const, reaction: 'test', updatedAt: now }],
...
async function main() {
  ...
  await step('unauthenticated access is rejected', async () => {
    const client = new ConvexHttpClient(backendUrl);
    await assert.rejects(client.query(api.health.snapshot, {}), /UNAUTHENTICATED/);
  });
  ...
  await step('profiles are owned and initialized', async () => {
    await alice.mutation(api.profile.save, { displayName: 'E2E Alice', goal: 'planning', onboardingCompleted: true, updatedAt: now });
    await bob.mutation(api.profile.save, { displayName: 'E2E Bob', goal: 'cycle', onboardingCompleted: true, updatedAt: now });
  });
  ...
  await step('updatedAt conflict and tombstone converge', async () => {
    await alice.mutation(api.health.syncBatch, {
      ...emptyBatch(),
      medications: [{ ...batch.medications[0], name: 'stale', updatedAt: now - 1 }],
    });
    let snapshot = await alice.query(api.health.snapshot, {});
    assert.equal(snapshot.medications[0]?.name, 'E2E препарат');
    await alice.mutation(api.health.syncBatch, {
      ...emptyBatch(),
      medications: [{ ...batch.medications[0], deletedAt: now + 2, updatedAt: now + 2 }],
    });
  ...
  await step('device-only URI fields are rejected and absent', async () => {
    await assert.rejects(
      alice.mutation(api.health.syncBatch, {
        ...emptyBatch(),
        scanResults: [{ ...batch.scanResults[0], localImageUri: 'file:///private/e2e.png' }],
      } as never),
    ...
    assert.equal(snapshot.chatMessages[0]?.attachments[0]?.availableLocally, false);
  });

  await step('account deletion blocks data and restore recovers it', async () => {
    await alice.mutation(api.account.requestDeletion, {});
    await assert.rejects(alice.query(api.health.snapshot, {}), /ACCOUNT_PENDING_DELETION/);
    assert.equal((await alice.query(api.account.status, {})).pendingDeletion, true);
    assert.equal((await alice.mutation(api.account.restore, {})).restored, true);
    ...
    await writeFile(
      ...
      JSON.stringify({ runId, backendUrl, startedAt, durationMs: Date.now() - startedAt, status: failure ? 'failed' : 'passed', steps }, null, 2),
    );
  }
