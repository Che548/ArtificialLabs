...
describe.sequential('AI chat consent and generation boundary', () => {
  ...
  test('defaults on, persists off, and can be re-enabled without fabricating consent', async () => {
    ...
    await expect(t.mutation(api.chat.setEnabled, { enabled: false })).rejects.toThrow('UNAUTHENTICATED');
    expect(await client.query(api.chat.status, {})).toMatchObject({ userEnabled: true, consentAccepted: false });
    await client.mutation(api.chat.setEnabled, { enabled: false });
    expect(await client.query(api.chat.status, {})).toMatchObject({ userEnabled: false });
    await expect(client.action(api.chat.generate, generationArgs('disabled_by_user'))).resolves.toEqual({ ok: false, code: 'USER_DISABLED' });
    expect(providerMock).not.toHaveBeenCalled();
    await client.mutation(api.chat.setEnabled, { enabled: true });
    expect(await client.query(api.chat.status, {})).toMatchObject({ userEnabled: true, consentAccepted: false });
    expect((await client.query(api.chat.status, {})).acceptedAt).toBeUndefined();
    await expect(client.action(api.chat.generate, generationArgs('enabled_by_user'))).resolves.toMatchObject({ ok: true });
  });
  ...
  test('disabling one account does not disable another', async () => {
    ...
    expect(await second.client.query(api.chat.status, {})).toMatchObject({ userEnabled: true });
  });
  ...
  test('does not deliver an in-flight answer after the user disables chat', async () => {
    ...
    providerMock.mockImplementationOnce(async () => {
      await client.mutation(api.chat.setEnabled, { enabled: false });
      return { ok: true as const, reply: 'Тестовый ответ', provider: 'yandex-ai-studio' as const, model: 'test', responseId: 'test', inputTokens: 1, outputTokens: 1, totalTokens: 2, durationMs: 1, truncated: false };
    });
    await expect(client.action(api.chat.generate, generationArgs('in_flight'))).resolves.toEqual({ ok: false, code: 'USER_DISABLED' });
  });
  ...
  test('preserves a legacy revocation and supports revocation before first use', async () => {
    ...
    await t.run((ctx) => ctx.db.insert('aiChatConsents', {
      userId, provider: 'yandex-ai-studio', policyVersion: AI_CHAT_CONSENT_POLICY_VERSION,
      acceptedAt: 1, revokedAt: 2, updatedAt: 2,
    }));
    expect(await client.query(api.chat.status, {})).toMatchObject({ userEnabled: false });
    const fresh = await createUser(t, 'fresh-off@example.test');
    await fresh.client.mutation(api.chat.revokeConsent, {});
    expect(await fresh.client.query(api.chat.status, {})).toMatchObject({ userEnabled: false, consentAccepted: false });
  });
